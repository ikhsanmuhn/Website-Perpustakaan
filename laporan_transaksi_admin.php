            ...
            <div class="sidebar-sticky">
                ...
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        ...
                        <a class="dropdown-item nav-link active" href="laporan_transaksi_admin.php" style="background:none;">Transaksi</a>
                        <a class="dropdown-item nav-link" href="laporan_terlambat_admin.php" style="background:none;">Terlambat</a>
                    </div>
                </li>
                </ul>
            ...
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
                ...
                <?php
                    ...
                    if(isset($_POST['search']) AND $_POST['start_date'] !="" AND $_POST['end_date'] !=""){
                        ...
                        $bulan = date('m');
                        $query = "SELECT t.idTransaksi, t.nis, s.nama, Concat(s.tingkat,' ',s.jurusan,' ',s.kelas)as kelas_nyatu, t.tglPinjam, MONTH(t.tglPinjam), t.qty, t.idPustakawan  FROM siswa s, transaksi t WHERE MONTH(t.tglPinjam) = :bulan AND t.nis = s.nis";
                        $stmt = $db->prepare($query);
                        $stmt->bindParam('bulan',$bulan);
                        $stmt->execute();
                        $cari = 1;
                    }
                    $mm = array('1' =>'JANUARI', '2'=>'FEBRUARI', '3'=>'MARET', '4'=>'APRIL', '5'=>'MEI', '6'=>'JUNI', '7'=>'JULI', '8'=>'AGUSTUS', '9'=>'SEPTEMBER', '10'=>'OKTOBER', '11'=>'NOVEMBER', '12'=>'DESEMBER');
                    $bebas=0;
                    while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                    $bul = $row['MONTH(t.tglPinjam)'];
                ...
                ?>
                        <div class="col-md-12 d-flex justify-content-center">
                            ...
                            ?>
                            <h4 style="margin-bottom:30px;">Laporan Transaksi dari <?php echo $mm["$bul"];?> tanggal <?php echo $from;?> / <?php echo $to;?></h4>
                            <?php
                                }elseif ($cari==1) {
                             
                            ?>
                            <h4 style="margin-bottom:30px;">Laporan Transaksi dari <?php echo $mm["$bul"];?> tanggal s/d</h4>
                            <?php
                                }
                            ?>
                        </div>
                        <div class="form">
                            <form method="POST" action="">
                                <div class="form-row">
                                    ...
                                    <input type="date" name="start_date" id="start" class="form-control" placeholder="Start Date">
                                    </div>
                                    <label for="end" class="col-md-2">End Date</label>
                                    <div class="form-group col-md-3">
                                    <input type="date" name="end_date" id="end" class="form-control" placeholder="End Date">                                    
                                    </div>
                                    <button class="btn btn-outline-secondary my-2 my-sm-0 col-md-2 " name="search" type="submit" style="border-radius:50px; height:40px;">Search</button> 
                                </div>
                            </form>
                        </div>
